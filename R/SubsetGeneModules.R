...
#' @param selected_gene_ratio ratio of selected genes which must be present in order for the module to be kept
#' @export
SubsetGeneModules <- function(gms, selected_genes, keep_mod_ID = FALSE, selected_gene_ratio = 0) {
  outlist <- list()
  ...
  gms = gms[unlist(lapply(gms, function(x) sum(x %in% selected_genes) >= round(length(x)*selected_gene_ratio)))]
